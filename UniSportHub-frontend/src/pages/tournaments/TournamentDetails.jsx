...
import { deleteTournament, getTournament, getTournamentTeams } from "../../api/tournamentService";
import Button from "../../components/common/Button";
import Loader from "../../components/common/Loader";
import ConfirmDialog from "../../components/common/ConfirmDialog";
import { extractError, formatDate, formatPrize, getImageUrl } from "../../utils/helpers";
...
function TournamentDetails() {
  ...
  const { data: teams, loading: teamsLoading } = useFetch(fetchTeams, [id]);
    ...
    <div className="space-y-6 animate-in">
      ...
      <div className="card p-0 overflow-hidden">
        {imgSrc ? (
          ...
          <div className="p-6" style={{ background: "linear-gradient(135deg,#0c4a6e,#1e293b)" }}>
            <h1
              style={{
                fontFamily: "'Bebas Neue', cursive",
                fontSize: "2.5rem",
        ...
        <div className="px-6 py-3 flex items-center gap-3 justify-end" style={{ borderTop: "1px solid #334155" }}>
          {isAdmin && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: "Created",
            value: tournament.createAt ? formatDate(tournament.createAt.slice?.(0, 10)) : "-",
          },
        ].map(({ label, value }) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {teamList.map((team) => (
              <Link
                ...
                onMouseEnter={(event) => {
                  event.currentTarget.style.borderColor = "rgba(14,165,233,0.4)";
                }}
                onMouseLeave={(event) => {
